...
export const POST = handle(async (req: Request, { params }: { params: Promise<{ id: string }> }) => {
  ...
  const video = await finalizeUpload(id, { durationMs: input.duration_ms, width: input.width, height: input.height, fileSize: input.file_size, mimeType: input.mime_type, hasAudio: input.has_audio }, admin.id);
