...
- T06 의 V02 기록은 V01 내용(반죽, 손 잡고 걷기)을 기술한 영상 착오다. **2026-09-22 연구자 지시로 무효화(결측 처리)**: `invalidate_and_retry(create_retry=false)` 로 플래그만 설정, 새 attempt 없음, 참여자 T06 은 유효 유지. 유효 기록 99/100, V02 19/20. 해당 코딩 세션(초안)은 코딩 대상에서 사라지며 확정하지 않는다.
- Reference 누락 후보: V05 "굴러간 양배추를 따라가 다시 잡는다"(교사 9명 공통), V02 "냄비에서 블록 붓기". Reference 를 보완하면 해당 초안을 재검토한다.
- 텍스트 지표 결측: 0자 3건(T06-V03, T21-V03, T21-V04) + 무효화 2건(T06-V02 영상 착오, T21-V02 미완성 20자 timeout — 2026-09-24 연구자 지시로 무효화, 새 attempt 없음) → 유효 기록 98/100(V02 18/20), 코딩 대상 110건.
...
- **support_type `observed_unreferenced` (마이그레이션 0012·0013, 연구자 지시 2026-09-22)**: 영상에 실제로 나타나지만 미미·순간적인 동작이라 Reference 에 기재되지 않은 사실적 기술. 관찰자가 세부를 묘사할 때 나타나며, 영상에서 전혀 확인되지 않는 `hallucination` 과 구분해 해석한다. 지표: factual = observed + observed_unreferenced + hallucination, precision = (observed + observed_unreferenced)/factual, hallucination_rate = hallucination/factual, **unreferenced_rate** = observed_unreferenced/factual (신규). recall·Human Detection Rate 는 Reference 대응이 있는 observed 만 사용. 미확인 초안 중 hallucination 으로 분류됐던 행은 `claude-fable-5-1/draft-v2` 로 이 범주로 재분류했고(V02-T06 은 다른 영상 내용이라 hallucination 유지), 코더 메모에 `[초안 v2]` 표시가 있다. 영상으로 확인되지 않는 경우 코더가 hallucination 으로 되돌린다.
